node{
      def dockerImageName= 'khushnood028/javadedockerapp_$JOB_NAME:$BUILD_NUMBER'
      stage('SCM Checkout'){
         git 'https://github.com/khushnood-hub/java-groovy-docker'
      }
      stage('Build'){
     ...
     stage('Build Docker Image'){         
      ...
      stage('Publish Docker Image'){
         withCredentials([string(credentialsId: 'dockerpwd', variable: 'dockerPWD')]) {
              sh "docker login -u khushnood028 -p ${dockerPWD}"
         }
